...
- Analytics begins firing while a child profile is active. *Current control:* `track()` returns early when kids mode is active, asserted **on the transport** in `kidsPrivacy.test.ts` with a positive control proving the assertion can observe a real send.
- Crash reporting begins firing while a child profile is active. *Current control:* Sentry is closed when kids mode is entered and its `beforeSend` discards events (`src/lib/monitoring.native.ts`). **This control is implemented in code but is NOT yet covered by an automated test**, unlike the analytics one. [OWNER ACTION: write it, or accept the gap knowingly.]
- A screen a child can reach makes a network request. *Current control:* `kidsPrivacy.test.ts` walks the `KID_ALLOWED` route list and requires a kids-mode gate next to every outbound call, with a documented exception for the parent-initiated "Forgot PIN" login. This check was added after a real leak: the child's playback screen requested a signed audio URL on every play, sending the account token and the exact chosen title, because the earlier test scanned only the `features/kids/` folder.
